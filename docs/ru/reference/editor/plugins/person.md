...
| `persons[].name` | да | Строка с именем. Профиль без имени сохраняется, если заполнены аватар, роль, биография или допустимая ссылка. |
...
Если редактор размещён в документе, отличном от документа библиотеки обрезки, или строгий режим `Trusted Types` блокирует панель обрезки, интерактивная обрезка пропускается. Используется исходное изображение; в `uploadFile` сохраняются его имя и MIME-тип. Политика `Trusted Types` по умолчанию не устанавливается.
