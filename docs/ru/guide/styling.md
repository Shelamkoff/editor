...
Rector предоставляет устойчивые корневые классы и пользовательские свойства CSS. Импортируйте CSS пакета до переопределений приложения и ограничивайте свои правила контейнером, чтобы несколько редакторов могли независимо использовать разные оформления.

## Обязательная таблица стилей

```js
import '@shelamkoff/rector/styles/editor.css'
```

Точка входа включает разметку редактора, CSS встроенных плагинов и переменные светлой и тёмной тем. Расширения со стилями из другого пакета подключают их через реестр Rector с подсчётом ссылок. Например, inline-плагин Color регистрирует таблицу стилей средства выбора цвета автоматически; импортировать её вручную не нужно.
...
Rector подсчитывает владельцев одинаковых URL между экземплярами и удаляет подключённый `<link>` после уничтожения последнего владельца. Не удаляйте такие элементы вручную.
...
У итогового отображения отдельный жизненный цикл стилей. Не связывайте опубликованное содержимое с классами интерфейса редактора.
...
`classPrefix` меняет пространство имён результата. Если стилями управляет приложение, импортируйте `@shelamkoff/rector/styles/renderer.css` и таблицы каждого выбранного рендерера.
