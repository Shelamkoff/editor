...
| `onDiagnostic` | нет | не задан | Получает служебные сигналы без содержимого документа в микрозадаче после завершения управляющего потока, создавшего сигнал. Если обработчик не передан, диагностика не вызывает код приложения. |
...
`onReady` ставится в очередь микрозадач после успешной сборки. `createEditor()` уже синхронно возвращает готовый дескриптор; обработчик нужен для уведомления другой части приложения. Если редактор уничтожен до выполнения микрозадачи, обработчик не вызывается. Обработчик может вернуть `Promise`; синхронное исключение или отклонение `Promise` изолируется и записывается как предупреждение, не меняя состояние готовности редактора.
...
Если внутренний `save()` выбрасывает исключение, Rector записывает предупреждение и не вызывает `onChange`. Обработчик может вернуть `Promise`; Rector наблюдает его, поэтому синхронное исключение и отклонение `Promise` записываются как предупреждение вместо необработанной ошибки. Это по-прежнему уведомление, а не транзакция хранения: более новое изменение может запланировать следующий вызов, пока предыдущий асинхронный запрос хранения ещё выполняется.
...
`validationMode` определяет поведение, когда `validate(data)` зарегистрированного плагина возвращает `false`. `onValidationError` и `onDiagnostic` являются наблюдательными обработчиками: они могут возвращать Promise, а их синхронные и асинхронные ошибки изолируются от проверки и управляющего потока редактора.

Далее `validationMode` применяет следующую политику:
