...
`element` и `contentElement` предназначены для измерения и интеграции. Прикладной код не должен менять через них сохраняемое содержимое. `version` изменяется, когда Rector помечает блок изменённым; его можно использовать как признак наблюдения, но это не версия документа. Сохранённое представление следует за атомарным преобразованием с тем же идентификатором, поскольку `convert()` сохраняет идентичность живого блока. После зафиксированного удаления такое представление окончательно становится недействительным и всегда выбрасывает исключение. Повторное использование той же строки `id` создаёт новое публичное представление; старый удалённый дескриптор не «оживает» для нового блока.
