...
События создаются синхронно для наблюдения. Не выполняйте тяжёлую работу прямо в обработчике; планируйте её отдельно. `editor:willChange` предназначено только для наблюдения: запуск другой команды документа из этого обработчика отклоняется, поскольку структурная команда уже могла зафиксировать идентификаторы блоков и индексы текущей транзакции. Пока транзакция команды активна, `undo()` и `redo()` возвращают `false`, а `setReadOnly()` и `destroy()` отклоняются до её завершения. Для сохранения используйте сериализованный документ из `onChange`.
