...
Свойство `readOnly` сообщает текущий режим. `setReadOnly(true)` отключает интерактивное редактирование существующего экземпляра, а `setReadOnly(false)` снова подключает инфраструктуру редактирования. Передача уже установленного значения ничего не делает. Значение не логического типа приводит к `TypeError`. Изменение режима во время активной транзакции команды отклоняется до перестройки каких-либо ресурсов режима редактирования.
...
`element` и `contentElement` предназначены для измерения и интеграции. Прикладной код не должен менять через них сохраняемое содержимое. `version` изменяется, когда Rector помечает блок изменённым; его можно использовать как признак наблюдения, но это не версия документа. Сохранённое представление разрешает текущий живой блок по идентификатору. Пока такого идентификатора в документе нет, обращение выбрасывает исключение; если тот же идентификатор позднее появится снова, удерживаемое приложением представление сможет разрешить новый живой блок. После зафиксированного удаления Rector удаляет собственную ссылку из кэша, чтобы удалённые идентификаторы не накапливались; поэтому последующий запрос через `editor.blocks` для повторно использованного идентификатора может вернуть другой объект представления.
...
События создаются синхронно для наблюдения. Не выполняйте тяжёлую работу прямо в обработчике; планируйте её отдельно. `editor:willChange` предназначено только для наблюдения: изменение состояния редактора из этого обработчика отклоняется, поскольку структурная команда уже могла зафиксировать идентификаторы блоков, индексы, фокус или выделение текущей транзакции. Пока транзакция команды активна, публичные изменения блоков, выделения и фокуса отклоняются, `undo()` и `redo()` возвращают `false`, а `setReadOnly()` и `destroy()` недоступны до её завершения. Для сохранения используйте сериализованный документ из `onChange`.
