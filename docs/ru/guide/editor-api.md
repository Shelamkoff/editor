...
`element` и `contentElement` предназначены для измерения и интеграции. Прикладной код не должен менять через них сохраняемое содержимое. `version` изменяется, когда Rector помечает блок изменённым; его можно использовать как признак наблюдения, но это не версия документа. Сохранённое представление продолжает указывать на блок после атомарного преобразования или замены документа, пока тот же идентификатор остаётся в текущем документе. После зафиксированного удаления представление окончательно становится недействительным; если отмена или последующий `render()` вернёт ту же строку идентификатора, получите новое представление через `editor.blocks`.
