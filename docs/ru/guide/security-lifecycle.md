...
Rector поддерживает страницы с `Content-Security-Policy: require-trusted-types-for 'script'`. Внутренние HTML-sink'и проходят через именованные scoped-policy вместо установки default policy. Если приложение дополнительно задаёт явный allowlist директивой `trusted-types`, разрешите политики `rector` и `dompurify`: `rector` применяется к разметке, принадлежащей Rector или уже прошедшей очистку, а `dompurify` используется очистителем Raw HTML.

Rector никогда не устанавливает default policy. `sanitizeHtml()` намеренно возвращает `string`, потому что очищенный HTML также сохраняется в данных документа и может отправляться в хранилище или по сети. Прикладной код не должен брать эту строку и напрямую присваивать её HTML-sink'у, защищённому Trusted Types. Предпочитайте построение DOM либо создавайте и отдельно проверяйте policy приложения для конкретной интеграции.

Значение Trusted Types само по себе не является решением об авторизации или доверии содержимому. Сохранённый или пользовательский HTML всё равно должен пройти подходящий sanitizer до передачи во внутренние trusted-sink helpers Rector. Иконки плагинов и другая разметка, поставляемая расширениями, выполняется с тем же уровнем доверия приложения, который описан в разделе [Доверие к расширениям](#доверие-к-расширениям).
