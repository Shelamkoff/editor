...
- актуальная стабильная версия браузера `Chrome` или `Chromium` для проверяемой при выпуске среды редактирования;
- Node.js 20.19+ или 22.12+ для установки и сборки;
- приложение, способное импортировать CSS и пакеты ESM.

Rector зависит от нативного поведения `contenteditable`, выделения и диапазонов, буфера обмена, фокуса, клавиатуры и составного ввода. Автоматическая проверка перед выпуском сейчас подтверждает эти контракты в `Chrome` и `Chromium`. Браузеры `Firefox` и `Safari`, а также движок `WebKit` могут работать, но не входят в поддерживаемый браузерный контракт до появления эквивалентных нативных автоматических проверок для этих движков.
