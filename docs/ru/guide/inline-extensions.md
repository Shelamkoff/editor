...
Контекст действий принадлежит одному открытию панели. Закрытие, сброс или замена панели делают его обработчики недействительными: последующий вызов не изменит выделение новой панели и не закроет её интерфейс. Удаление исходного редактируемого поля также запрещает изменение документа. Контекст из `onMount()` принадлежит конкретному экземпляру панели инструментов и становится недействительным при её уничтожении, в том числе при переключении режима только для чтения.

Оба метода `mutate()` возвращают `T | undefined`: действующий вызов возвращает результат операции, а устаревший — `undefined`, не выполняя операцию. Код, использующий возвращаемое значение, должен учитывать этот случай. Прямые изменения DOM остаются ответственностью расширения; выполняйте их внутри переданного обработчика изменения.
