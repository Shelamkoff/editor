...
Синхронная точка входа рендерера содержит полный встроенный набор. Её граф модулей включает интеграции Gallery и Person, поэтому до импорта `@shelamkoff/rector/renderer` установите их необязательные равноправные зависимости:
...
| `validationMode` | `preserve` | сохранить ошибочные данные встроенного блока или выбросить исключение до отображения |
...
Строгая проверка сверяет структуры данных и правила URL встроенных блоков до вызова их рендереров. Пользовательский рендерер с тем же типом владеет собственным контрактом проверки, поэтому замена встроенного рендерера одновременно отключает его встроенную проверку.
...
Если у блока нет `revision`, Rector сравнивает полную сигнатуру его `type`, `data` и `inline`. Источник данных, уже поддерживающий достоверную редакцию содержимого или устойчивый хеш, может передать его в `block.revision`. Тогда Rector сравнит это значение за постоянное время без сериализации данных блока:
...
Меняйте `revision` при каждом изменении `data` или `inline`. Одинаковая редакция для разного содержимого намеренно сообщает рендереру, что блок не изменился. Редактор сохраняет входящую редакцию, пока блок не тронут, и удаляет её после первого локального изменения, потому что не может создать следующую редакцию от имени источника данных.
