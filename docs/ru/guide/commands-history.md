...
`Mod` означает Command в macOS и Control в Windows или Linux. В версии 1.0.0 публичных методов `undo()` и `redo()` намеренно нет. Кнопки приложения не должны обращаться к внутреннему диспетчеру истории.
...
`history:commit` сообщает о записанном шаге, а `editor:changed` — об изменении документа. `onChange` — отложенное сериализованное уведомление, предназначенное для сохранения.
