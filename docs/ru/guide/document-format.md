...
- `tunes` содержит необязательные общие настройки блока.
...
- `preserve` принимает структурно корректный документ неизвестной версии, если с этой версии не начинается миграция;
...
Rector копирует вход перед каждым шагом, проверяет, что результат является документом, и принудительно задаёт объявленную версию `to`. Повторяющиеся `from`, переход в ту же версию, циклы и неполные цепочки отклоняются в соответствии с выбранной политикой.
