...
| `validationMode` | `preserve` | keep invalid built-in data or throw before rendering it |
...
Strict validation covers built-in data shapes and URL policies before their built-in renderers run. A custom renderer registered for the same type owns its own validation contract, so replacing a built-in renderer also disables that built-in validator.
...
When a block has no `revision`, Rector compares a deep signature of its `type`, `data`, and `inline` values. A producer that already maintains an authoritative content revision or stable hash may supply it as `block.revision`. Rector then compares that value in constant time instead of serializing the block data:
