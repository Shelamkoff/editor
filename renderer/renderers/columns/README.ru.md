...
Преобразует сохранённый блок `columns` в принадлежащий рендереру DOM.
...
Содержимое каждой колонки проходит общий обработчик внутристрочной разметки, а проверенная раскладка определяет число и пропорции колонок. Рендерер объявляет одну таблицу стилей и не создаёт обработчиков или сторонних экземпляров.
