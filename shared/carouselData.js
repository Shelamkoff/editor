...
const ASPECT_RATIO_RE = /^(?:auto|\d+(?:\.\d+)?\s*\/\s*\d+(?:\.\d+)?)$/
...
export function normalizeCarouselData(input, createId) {
  ...
  const options = {
    ...
    autoplayDelay: Number.isFinite(rawOptions.autoplayDelay) && rawOptions.autoplayDelay > 0
      ? Math.floor(Number(rawOptions.autoplayDelay))
      : 3000,
    navigation: rawOptions.navigation !== false,
    pagination: rawOptions.pagination !== false,
    thumbnails: rawOptions.thumbnails === true,
  }
  if (typeof rawOptions.aspectRatio === 'string' && ASPECT_RATIO_RE.test(rawOptions.aspectRatio.trim())) {
    options.aspectRatio = rawOptions.aspectRatio.trim()
  }
...
export function validateCarouselData(data) {
  ...
  for (const slide of value.slides) {
    ...
    ids.add(slide.id)
    if (!['image', 'video', 'html'].includes(/** @type {any} */ (slide.type))) return false
  ...
  if (!Number.isFinite(value.options.autoplayDelay) || value.options.autoplayDelay <= 0) return false
  return value.options.aspectRatio === undefined
    || (typeof value.options.aspectRatio === 'string' && ASPECT_RATIO_RE.test(value.options.aspectRatio.trim()))
}
