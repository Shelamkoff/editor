...
Bookmark preview with seven visual templates and optional application-provided metadata.
...
`fetchMeta?: (url: string, context: { signal: AbortSignal }) => Promise<{ title?: string; description?: string; image?: string; favicon?: string; domain?: string }>` resolves metadata in application code and must stop work when the supplied signal is aborted.

## Capabilities

URL paste; async metadata; template selector; safe URL assignment and cleanup.
