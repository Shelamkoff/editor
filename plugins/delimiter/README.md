...
The registered block type is `delimiter`. The class is also exported by the complete `@shelamkoff/rector/plugins` preset and can be loaded through `@shelamkoff/rector/plugins/async`.
...
Use the matching renderer from `@shelamkoff/rector/renderer/renderers/delimiter`. The VitePress guide documents configuration, commands and history, extension contracts, document migrations, styling, security, and lifecycle in a sequential form.
